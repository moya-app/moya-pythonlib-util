...
class KafkaProducer(KafkaBase):
    ...
    async def send_nowait(
        self, topic: str, payload: dict, timestamp_ms: int = None
        ...
        fut = await self.kafka.send(topic, json.dumps(payload).encode("utf-8"), timestamp_ms=timestamp_ms)
        # return t.cast(asyncio.Future[aiokafka.RecordMetadata], fut)
        return t.cast(asyncio.Future, fut)

    async def send(self, topic: str, payload: dict, timestamp_ms: int = None) -> t.Any:  # aiokafka.RecordMetadata:
        """
        ...
        """
        return await self.send_nowait(topic, payload, timestamp_ms)
